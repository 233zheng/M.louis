...
local function castFn(inst, target)
    ...
    if body then
        owner.components.inventory:DropItem(body)
    end
...
local function fn()
    ...
    inst.components.weapon:SetDamage(TUNING.KATANA.HARAKIRIDMG)

    inst:AddComponent("finiteuses")
    inst.components.finiteuses:SetMaxUses(TUNING.KATANA.HARAKIRI_USES)
    inst.components.finiteuses:SetUses(TUNING.KATANA.HARAKIRI_USES)
